...
export class TicketsRepository extends Repository {
  ...
  public async create(ticket: Ticket): Promise<number> {
  ...
  public async getByExternalId(externalId: string): Promise<Ticket|null> {
    const statement = "SELECT id, violator_id, external_id, lot_id, make, model, tag, plate_state_id, amount, issued_at, violation_type_id "
